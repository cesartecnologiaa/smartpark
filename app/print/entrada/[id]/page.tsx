...
export default function PrintEntradaPage({ params }: { params: { id: string } }) {
  ...
  const styles = useMemo(
    () => ({
      ...
      labelTop: is58 ? '11px' : '13px',
      codeFont: is58 ? '32px' : '42px',
      rowFont: is58 ? '12px' : '14px',
      footerFont: is58 ? '10px' : '12px',
      qrSize: is58 ? '88px' : '120px',
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const nextSettings = settingsSnap.exists() ? (settingsSnap.data() as EstablishmentSettings) : null;
        ...
        <div className="print-ticket-page">
          <div className="print-ticket">
            ...
            <div className="ticket-footer">
              ...
              <p>Perda do ticket: taxa adicional será cobrada.</p>
            </div>
...
function buildStyles(styles: PrintStyleMap) {
  return `
    ...
    .ticket-label-top { text-align: center; font-size: ${styles.labelTop}; font-weight: 700; letter-spacing: .08em; }
    .ticket-code { text-align: center; font-size: ${styles.codeFont}; font-weight: 800; line-height: 1; margin: 8px 0 10px; }
    .ticket-qr-wrap { display: flex; justify-content: center; margin: 8px 0; }
    .ticket-qr { width: ${styles.qrSize}; height: ${styles.qrSize}; object-fit: contain; }
